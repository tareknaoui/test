...
class ImgStg():
    ...
    def _merge_txt_rgb(self, txt_bits, rgb):   
        ...
        r_new = r[:-1]+ txt_bits[0]
        g_new = g[:-1]+ txt_bits[1]
        b_new = b[:-1]+ txt_bits[2]

        if r != r_new or g != g_new or b != b_new:  # Check if any color value has changed
            print(f'Changed pixel: avant - R={r}, G={g}, B={b}; apres - R={r_new}, G={g_new}, B={b_new}')

        new_rgb = self._bin_to_int((r_new,g_new,b_new))
        return new_rgb
    ...
    def get_text_pixel_count(self, img_p):
        img = Image.open(compatible_path(img_p))
        text_len = self._resolve_place_len(img) * 16  # get text length in bits
        pixel_count = text_len // 3  # each pixel can store 3 bits of text
        
        return pixel_count
    def print_text_pixel_count(self, img_p):
        pixel_count = self.get_text_pixel_count(img_p)
        print(f'pixels necessaire  {pixel_count} pixels.')
...
if __name__ == "__main__":
    ...
    stg.save_img(destPath, EmbededImagePath)  # Save the modified image
    stg.print_text_pixel_count(EmbededImagePath)
